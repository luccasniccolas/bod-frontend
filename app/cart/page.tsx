              ...
              {cartItems.map((item) => (
                <li key={item.id} className="flex flex-col min-[500px]:flex-row min-[500px]:items-center gap-5 py-6 border-b border-gray-200 group">
                  <div className="w-full md:max-w-[126px]">
                    <img src={`https://via.placeholder.com/150`} alt="product image" className="mx-auto" />
                  </div>
                ...
                <div className="mt-8">
                    ...
                    <p className="font-medium text-lg leading-8 text-black">${cartItems.reduce((total, item) => total + item.price * item.quantity, 0)}</p>
                    </div>
                    {/* ... resto del código HTML para el resumen del pedido ... */}
                </div>
            </div>
        </div>
    </div>
